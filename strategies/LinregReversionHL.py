...
class LinregReversionHL(Strategy):
    ...
    def getName(self):
        return 'LinregReversionHL'

    def bull(self, df, index=-1):
        self.addConditional(linreg.lin_reg_reversion_2deviations_bull(df, period=self.period, index=index-1, size=self.size))
    ...
    def bear(self, df, index=-1):
        self.addConditional(linreg.lin_reg_reversion_2deviations_bear(df, period=self.period, index=index-1, size=self.size))
        if self.isfollowtendency:
