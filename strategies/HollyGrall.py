...
class HollyGrall2(Strategy):
    ...
    def getIndexMAIntercept(self, df, index=-1, size=4, bull=True):
        for i in range(0,size):
            index = index-1
            if bull:
    ...
    def bull(self, df, index=-1):
        ...
        if self.intercept != None:
            if rules.adx_up(df, period=self.adx_p, index=self.intercept, adx_length=self.adx_length):
                if self.is_candles_follow_ma(df, index=self.intercept, bull=True):
                    highest = self.getHighestTopOrBottonIndex(df, index=self.intercept-1, size=5, bull=True)
                    if df.iloc[index]['close'] > df.iloc[highest]['high']:
                        return True
        return False
    ...
    def bear(self, df, index=-1):
        ...
        if self.intercept != None:
            if rules.adx_up(df, period=self.adx_p, index=self.intercept, adx_length=self.adx_length):
                if self.is_candles_follow_ma(df, index=self.intercept, bull=False):
                    lowest = self.getHighestTopOrBottonIndex(df, index=self.intercept-1, size=5, bull=False)
                    if df.iloc[index]['close'] < df.iloc[lowest]['low']:
                        return True
        return False

    def hitStopGain(self, index=-1):
        if self.trend:
            if self.df.iloc[index]['close'] > self.startprice and rules.is_ema_up(self.df, self.ema, attr='high', index=index):
                self.stopgain = self.df.iloc[index]['close']
                return True
        else:
            if self.df.iloc[index]['close'] < self.startprice and rules.is_ema_down(self.df, self.ema, attr='low', index=index):
                self.stopgain = self.df.iloc[index]['close']
                return True
        return False

    def buyStopLoss(self, index):
        return self.df.iloc[self.intercept]['low']
    ...
    def sellStopLoss(self, index):
        return self.df.iloc[self.intercept]['high']
